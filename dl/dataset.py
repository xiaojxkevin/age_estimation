...
class Age(Dataset):
    def __init__(self, data_dir:str, split_path:str) -> None:
        super().__init__()
        torch.manual_seed(3407)
        self.data_path = os.path.join(data_dir, "utkface_aligned_cropped/UTKFace")
...
class Utkface_for_stage(Dataset):
    def __init__(self, data_dir:str, split:str, stage = 0) -> None:
        super().__init__()
        torch.manual_seed(3407)
        self.split = split
        assert split in ("train", "val", "test")
        ...
        self.data_path = os.path.join(data_dir, "utkface_aligned_cropped/UTKFace")
        self.split_path = os.path.join(data_dir, "stage{}".format(stage), split+".txt")
        self.image_names = np.genfromtxt(self.split_path, dtype=str).tolist()
        self.trans = transforms.Compose([
            transforms.ColorJitter(brightness=0.2, contrast=0.1, saturation=0.1, hue=0.1),
            transforms.ToTensor(),
...
if __name__ == "__main__":
    ds = Age("./data", "data/young/files.txt")
    print(len(ds))
    img = ds[0]["image"]
    print(img.shape)
